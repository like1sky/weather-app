<template>
  <main class="container text-white">
    <div class="pt-4 mb-8 relative">
      <input @input="getSearchResults" v-model="searchQuery" type="text" placeholder="Search for a city or state"
      ...
      <ul v-if="mapboxSearchResults" class="absolute bg-weather-secondary text-white w-full shadow-md py-2 px-1 top-[66px]">
        ...
        <template v-else>
          ...
          class="py-2 cursor-pointer" @click="previewCity(searchResult)">{{ searchResult.place_name }}
          </li>
        </template>
      </ul>
    </div>
...
const previewCity = (searchResult)=>{
  ...
  router.push({
    ...
    query:{
      ...
      lng:searchResult.geometry.coordinates[0],//latitude 纬度
      preview:true,
    }
  })
}
