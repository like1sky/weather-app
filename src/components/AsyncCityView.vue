...
const getWeatherData = async ()=>{
    ...
    //  `https://api.openweathermap.org/data/2.5/onecall?lat=${route.query.lat}&lon=${route.query.lng}&exclude={part}&appid=d01c09638ee58360fdff1e816f0019da`);//我的为什么不行？？
        const weatherData = await axios.get(
      `https://api.openweathermap.org/data/2.5/onecall?lat=${route.query.lat}&lon=${route.query.lng}&exclude={part}&appid=7efa332cf48aeb9d2d391a51027f1a71&units=imperial`);//作者的
       
        //cal current date&time
        const localOffset = new Date().getTimezoneOffset()*60000;//getTimezoneOffset() 返回当前时区与UTC之间的分钟差，乘以60000将其转换为毫秒。
        const utc = weatherData.data.current.dt*1000+localOffset;//weatherData.data.current.dt 存储了当前时间的UNIX时间戳（以秒为单位），乘以1000将其转换为毫秒，并加上本地时区的偏移量。
        weatherData.data.currentTime = utc+1000*weatherData.data.timezone_offset;//将计算得到的UTC时间加上天气数据中的时区偏移量，并将结果存储在 weatherData.data.currentTime 中。

        //cal hourly weather offset
        weatherData.data.hourly.forEach(hour=>{
            const utc = hour.dt *1000+localOffset;//计算每小时天气数据的UTC时间
            hour.currentTime = utc + 1000*weatherData.data.timezone_offset;//UTC时间加上时区偏移量，并将结果存储在当前小时的 hour.currentTime 中。
        });
        //Flicke Delay
